...
class CipherSuite extends munit.FunSuite {
  val WORDS = List(
    "8-10 r: ozrcdfnug",
    "9-10 q: hvsazxrigf",
    "1-9 j: bbqonxjt",
  ...
  test("success word from example") {
    val word = "2-4 f: fgff"
    val result = Cipher.findInvalidCases(List(word))
    assertEquals(result.length, 0)
  }

  test("another success word from example") {
    val word = "1-6 h: hhhhhh"
    val result = Cipher.findInvalidCases(List(word))
    assertEquals(result.length, 0)
  }

  test("error word from example") {
    val word = "4-6 z: zzzsg"
    val result = Cipher.findInvalidCases(List(word))
    assertEquals(result.length, 1)
  }

  test("count invalid cases") {
    val result = Cipher.findInvalidCases(WORDS)
    assertEquals(result.length, 479)
  }

  test("42 position invalid case") {
    assertEquals(Cipher.findByIndex(WORDS, 41), Option("9-10 d: bgamidqewtbus"))
  }

  test("13 position invalid case") {
    assertEquals(Cipher.findByIndex(WORDS, 12), Option("7-10 i: nljzuyfzb"))
  }
}
