...
class MySuite extends munit.FunSuite {
  test("example one") {
    val expected = false
    val result = Database.isInvalid("1a421fa,alex,alex9@gmail.com,18,Barcelona")
    assertEquals(expected, result)
  }
  test("example two") {
    val expected = true
    val result = Database.isInvalid("9412p_m,maria,mb@hotmail.com,22,CDMX")
    assertEquals(expected, result)
  }
  test("example three") {
    val expected = false
    val result = Database.isInvalid("494ee0,madeval,mdv@twitch.tv,,")
    assertEquals(expected, result)
  }
  test("example four") {
    val expected = true
    val result = Database.isInvalid("494ee0,madeval,twitch.tv,22,Montevideo")
    assertEquals(expected, result)
  }
  test("challenge") {
    val cases = List(
    ...
    val code = Database.generateCode(cases)
    assertEquals(code, "dasd")
  }
}
